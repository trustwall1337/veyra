...
| 31c | **(D6) Sub-agent spawn gate + sub-scope derivation** — `policyGate.authorizeSpawn(proposal, policy, state, depth)` (depth-cap `depth>=1`→deny + policy + `TargetDescriptor` validation) + `deriveSubScope(target, catalog, policy)` (deterministic, strict-subset-guarded, `Result`-returning). Core gate half in `src/core/policy/`; scope-derivation reads only the catalog contract (core stays import-clean). | 30, 31, 33 | M |
| 41 | Fixture gate: agentic e2e + determinism + write-then-cleanup roundtrip both paths + trust-invariant assertions + per-tool failure-boundary assertion + **result-reject-boundary assertion** + required-evidence-ledger assertions (early `done` → per-missing `coverage_gap`; **ledger row-count constant pinned**) + **(D6) sub-agent assertions: depth-cap denial, budget no-escape, strict-subset, sub-agent-crash→`coverage_gap`+parent-continues, no trace row `subagent_depth>1`** | 31-40b, 31c | L |
...
## §O. Bounded deep-dive sub-agents (D6 addendum)

The single agentic loop (§B) may spawn **bounded deep-dive sub-agents** to thoroughly investigate ONE target. Deep-dive only (NOT parallel fan-out, NOT specialization). Sub-agents are strictly MORE constrained than the parent, never less. Every D6 constraint is enforced by a schema field / gate check / budget object — not prose.

**Spawn mechanism (chosen):** a third proposal arm `spawn_deep_dive` alongside `invoke_tool`/`done` (runner-up: a `deep-dive` meta-tool — rejected: hides the depth cap inside an `invoke` body the gate can't see). The gate adjudicates every proposal, so the depth cap is one gate predicate. Loop body delta:

```
if proposal.kind=='spawn_deep_dive':
    gate = policyGate.authorizeSpawn(proposal, policy, state, depth=current_depth)  # depth>=1 ⇒ always deny
    if not gate.allowed: state.recordSpawnDenial(...); continue
    subTarget = TargetDescriptor.safeParse(proposal.target_descriptor)  # closed typed union, no free text
    if not subTarget.ok: state.recordArgReject(...); continue
    subScope  = deriveSubScope(subTarget.value, parentScope, policy)    # strict subset of the PARENT's current scope (codex r1 #3)
    subBudget = budget.reserveChild(proposal.requested_slice)           # view over root budget; clamped to remaining
    try:                                                                # whole-call parent catch (codex r1 #1)
      runDeepDive(subTarget.value, subScope, subBudget, parent_step=step, depth=current_depth+1, state, policy, aiDriver)
    except cause:
      state.recordSubagentError(subagent_id, error_class=cause.constructor.name)  # target → §K coverage_gap; parent continues
    continue
```

**Failure isolation (codex r1 MUST #1).** The per-tool `try` inside `runDeepDive` catches tool throws, but a sub-agent can fail OUTSIDE `tool.invoke` (driver error, gate error, scope-derivation error). So the parent wraps the WHOLE `runDeepDive` call in a `try/catch` (shown above): any escape records `subagent_error`, the deep-dive target is marked unsatisfied for the §K ledger (→ floor `coverage_gap`), and the parent `continue`s. Two nested boundaries: per-tool (inside) + whole-sub-loop (parent-side). Step 41 asserts both — a sub-agent that throws outside any tool still leaves the parent able to reach its floor + cleanup.

**Strict-subset against parent scope (codex r1 #3).** `deriveSubScope` takes `parentScope` (the descriptors the parent can currently see), NOT the global `toolCatalog`. At depth 0 `parentScope` == full catalog; the distinction is load-bearing for consistency (a sub-agent can never see a tool its parent couldn't). Assertions: child scope `⊊ parentScope`, `size < parentScope.size`, every child descriptor object-identical (`===`) to a `parentScope` member, every child descriptor's `required_action` ∈ `policy.allowed_actions`.

`runDeepDive` is the SAME driver as `run`, parameterized by `(scope, ChildBudget, depth, parent_step)`. Parent `run` = `runDeepDive` with full catalog, root budget, depth 0. One driver, two entry depths. Sequential (parent blocks on return → deterministic).

**D6 constraints, mechanically:**
- **Depth cap = 1:** `authorizeSpawn` denies `depth>=1`. Sub-loop runs at depth 1, so its `spawn_deep_dive` is gate-denied before any nested call site. One integer compare in `src/core/policy/`.
- **Narrow subset:** `deriveSubScope` = pure function of `target.kind` via a checked-in `DEEP_DIVE_SCOPE_TABLE` (actions, not tool ids) filtered by `policy.allowed_actions`; result asserted a STRICT subset of the parent catalog (`⊊`, every member `===` a registered descriptor). No tool promotion via a sub-agent.
- **Budget no escape:** one root budget per scan; `ChildBudget` is a VIEW forwarding to root counters; `reserveChild` clamps `requested_slice` to `remaining_root`, cannot raise a cap. Sub-agent spend debits the root; first-trips-wins is global. A sub-agent can END the scan, never extend it.
- **Same trust spine:** sub-agent tool calls pass the SAME gate + `result_schema.safeParse` boundary; sub-agent `ToolResult`s are the whitelist-only fact-payload type (classification keys un-representable at any depth) → sub-agents emit facts only, never classify. Floor stays sole Finding producer.
- **Facts to parent:** sub-agent writes to the SAME append-only `ArtifactState` (tagged `subagent_id`+`parent_step`); no merge step; floor reads parent+sub facts identically.
- **Nested audit (§F delta):** trace rows gain `parent_step`, `subagent_id`, `subagent_target`, `subagent_depth` (∈{0,1}); test asserts no row `subagent_depth>1`. Operator reconstructs a deep-dive by filtering `subagent_id`.
- **Failure isolation:** `runDeepDive` wrapped in the same per-tool failure boundary; crash → `subagent_error` + the deep-dive target → §K `coverage_gap`; parent `continue`s. Append-only state cannot be corrupted.

**Tool-subset scoping (chosen):** deterministic `deriveSubScope` (runner-up: AI-proposed-then-validated — rejected: extra AI capability-surface + a round-trip for the same subset). `DEEP_DIVE_SCOPE_TABLE` is a checked-in literal keyed by `TargetDescriptor.kind`, CI-pinned row count.

**Sequencing:** Cut 1, last item before 41-partial. Depends on Step 31 (loop) + the gate + the result boundary + Step 33 (registered catalog to subset). In Cut 1 (Mode A read-only) a sub-agent's subset can contain NO write tool (policy forbids write actions) → near-zero new trust surface. The Mode-B write-capable deep-dive (e.g. `suspected_idor`) inherits Cut 3's unified write registry + cleanup for free — no extra sub-agent work.

**Budget test coverage (codex r1 MUST #2).** Step 41's budget tests must cover, not just `total ≤ root cap`: (a) `reserveChild(requested > remaining)` clamps to `remaining`; (b) `ChildBudget` cannot raise ANY root cap (immutability); (c) ALL cap dimensions share root state — `max_tool_calls`, `max_wall_clock_ms`, `max_ai_cost_units`, and `max_steps` each debit the root when a sub-agent spends; (d) sub-agent denials/arg-rejects/result-rejects/tool-errors count against the root budget; (e) `ChildBudget` holds no independent counter that could be reset/forgotten.

**Import-graph entrypoints from the registry (codex r1 #4).** The §D.2(iii) import-graph walk derives its entrypoint set from the exact registry object used to build BOTH parent and sub-agent scopes — registered descriptors carry source-module metadata, and the walk follows those modules transitively INCLUDING type-only imports (not filename globs). A sub-agent calls only registered tools, so every tool it can reach is in the walk's scope; `Finding` stays unreachable. **Layering note (codex):** keep `TargetDescriptor` schemas + `DEEP_DIVE_SCOPE_TABLE` rows provider-agnostic in `src/core` (actions + opaque refs, no provider-specific concrete schema) so `no-cross-layer-imports.test.ts` stays green; any provider-specific target shape lives in the leaf folder, not core.

**New work:** Step 31c (spawn gate + scope derivation). AMENDS Step 30 (`spawn_deep_dive` arm + `TargetDescriptor` type + `DEEP_DIVE_SCOPE_TABLE` + `ArtifactKind: subagent_error` + descriptor source-module metadata for the walk), Step 31 (depth-aware `runDeepDive` + `ChildBudget` + parent-side whole-call try/catch), Step 34 (4 trace fields), Step 41 (sub-agent assertions: depth-cap, full-dimension budget no-escape per codex r1 #2, strict-subset-vs-parent-scope, classification-leak, both failure boundaries, no `subagent_depth>1`). KEEPS result boundary, floor, §K ledger, write registry unchanged. Codex budget: 2 rounds (D6).
